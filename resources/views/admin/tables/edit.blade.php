<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Editar Mesa {{ $mesa->number }}</h2>
        </div>
    </x-slot>
                ...
                <form method="POST" action="{{ route('admin.mesas.update', $mesa) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <x-text-input id="capacity" name="capacity" type="number" class="mt-1 block w-full"
                            value="{{ old('capacity', $mesa->capacity) }}" required min="1" max="20" />
                    ...
                    <div>
                        ...
                        <select id="status" name="status"
                            ...
                            <option value="reserved" {{ $mesa->status === 'reserved' ? 'selected' : '' }}>Reservada</option>
                        </select>
                    </div>
                    <x-primary-button class="w-full justify-center">Actualizar mesa</x-primary-button>
                </form>
            </div>
        </div>
    </div>
