            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    @if(auth()->user()->role === 'admin')
                        ...
                        <x-nav-link :href="route('kitchen.index')" :active="false">
                            🍳 Cocina
                        </x-nav-link>
                    @else
                        <x-nav-link :href="route('waiter.tables')" :active="request()->routeIs('waiter.tables')">
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
                            ...
                            <span class="ml-2 text-xs px-2 py-0.5 rounded-full {{ auth()->user()->role === 'admin' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700' }}">
                                {{ auth()->user()->role === 'admin' ? 'Admin' : 'Mesero' }}
                            </span>
                            <div class="ms-1">
